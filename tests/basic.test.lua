...
Tx.describe("methods", function()
    ...
    Tx.it("should throw unknown method", function()
        Tx.throws(function()
            local trie = Trie.new()
            trie:insert("UNKNOWN", "/hello", function() return "bad" end)
        end)
    end)
end)
...
Tx.describe("priority", function()
    ...
    Tx.it("should choose best scored/specific route", function()
        local trie = Trie.new()
        trie:insert("GET", "/user/:1", function() return 1 end)
        trie:insert("GET", "/user/:1/:2", function() return 2 end)
        trie:insert("GET", "/user/:1/:2/:3", function() return 3 end)
        local x, p = trie:search("GET", "/user/p1/p2")
